...
export const Sidebar: React.FC<SidebarProps> = ({ isCollapsed }) => {
  ...
  return (
    <aside className={`bg-bg-secondary border-r border-gray-700 fixed left-0 top-0 h-screen z-20 transition-all duration-300 ${
      ...
      <div className="p-4">
        <div className={`text-primary font-bold text-xl mb-6 ${isCollapsed ? 'text-center' : ''}`}>
        ...
        {!isCollapsed && (
          <Link
            to="/sitemap-approval"
            className={`block w-full mb-6 px-4 py-3 rounded-lg text-sm font-medium text-center transition-all duration-200 ${
          ...
          {NAVIGATION_DATA.map((portal) => (
            <div key={portal.id} className="mb-4">
              <button
                ...
                className={`w-full flex items-center justify-between p-3 rounded-lg hover:bg-bg-hover text-left transition-all duration-200 ${
                  location.pathname.startsWith(`/${portal.id}`) ? 'bg-bg-hover text-primary' : 'text-gray-300'
                }`}
              >
                  ...
                  {portal.sections.map((section) => (
                    <div key={section.id}>
                      <button
                        ...
                        className="w-full flex items-center justify-between p-2 rounded-lg hover:bg-gray-600 text-left text-sm text-gray-400 transition-all duration-200"
                      >
                        ...
                        <div className="ml-4 mt-1 space-y-1">
                          {section.items.map((item) => (
                            <Link
                              key={item.id}
                              to={item.path}
                              className={`block p-2 rounded-lg text-xs hover:bg-gray-600 transition-all duration-200 ${
                                isActive(item.path)
